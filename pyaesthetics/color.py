...
def attr_color_complexity(img, save_path=None, filter = False, saturation_low = 0.2, value_low = 0.15, value_high=0.95):
    ...
    # Filter pixels based on saturation and value thresholds
    if filter == True:
        hsv_g = hsv_l[(hsv_l[:, 1] >= 255 * saturation_low) & (hsv_l[:, 2] >= 255 * value_low) & (hsv_l[:, 2] <= 255 * value_high)]
    else:
        hsv_g = hsv_l
    ...
    # Count occurrences of each unique HSV value
    hsv_counts = Counter(map(tuple, hsv_g))

    # Total number of valid pixels
    N = len(hsv_g)

    # Compute color variety using the provided formula
    C = -sum((count / N) * np.log(count / N) for count in hsv_counts.values())
    ...
    if isinstance(save_path, str):
        ...
        plt.title(f"Color Variety (C): {C:.4f}")
        plt.axis('off')
