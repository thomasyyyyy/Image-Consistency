...
def attr_blur_laplacian(img,
    ...
    d = {"blur_laplacian": blur}
    return(d)

def attr_blur_fft(img,
                  save_path = None,
...
def attr_dof_block(img,
             blur_method = attr_blur_fft, 
             return_raw = False):
    nblock = 4
    ntotal = nblock ** 2
    gray = misc.read_img_gray(img)
    windows = misc.split_block(gray, nblock)
    sharps = [  [*blur_method(x).values()][0] for x in windows] # get sharpness measures for each block
    sharps_ar = np.array(sharps)
    summary = summary_blur(sharps)

    index_keep = np.array([5,6,9,10])
    inners = sharps_ar[index_keep]  # get inner blocks

    dof_inner = np.mean(inners) / np.mean(sharps)
    rlist = [dof_inner] + summary[:]
    statnames = ["mean","median","std_dev","min","max",
                 "quartile_1","quartile_3","skew","kurtosis"]
    attributes = ["dof_inner"] + [ "blur_block_" + var for var in statnames]

    if return_raw:
        rlist = rlist[:] + sharps[:]
        attributes.extend( ["blur_block_" + str(x) for x in range(1, ntotal+1)]  ) 
